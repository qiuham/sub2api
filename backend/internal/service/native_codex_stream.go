...
// handleNativeCodexStream keeps the upstream SSE wire body intact while
// collecting response IDs and usage for the existing accounting path.
func (s *OpenAIGatewayService) handleNativeCodexStream(resp *http.Response, c *gin.Context, started time.Time) (*openaiStreamingResultPassthrough, error) {
	...
	result := &openaiStreamingResultPassthrough{usage: usage}
	...
	processEvent := func() {
		...
		if result.firstTokenMs == nil && string(data) != "[DONE]" {
			ms := int(time.Since(started).Milliseconds())
			result.firstTokenMs = &ms
		}
		resolvedType := effectiveOpenAISSEEventType(data, eventName)
		observer.ObserveOpenAI(data, resolvedType)
		s.parseSSEUsageBytesWithType(data, resolvedType, usage)
		images.AddSSEData(data)
		if result.responseID == "" {
			result.responseID = extractOpenAIResponseIDFromJSONBytes(data)
		}
		if string(data) == "[DONE]" || resolvedType == "response.completed" || resolvedType == "response.failed" || resolvedType == "response.done" {
			terminal = true
		}
		eventName = ""
		dataLines = dataLines[:0]
	}
	finish := func(err error) (*openaiStreamingResultPassthrough, error) {
	...
	for {
		...
		if len(fragment) > 0 {
			...
			if len(line)+len(fragment) > maxEventLine {
				return finish(fmt.Errorf("native SSE line exceeds %d bytes", maxEventLine))
			}
